 ...
 * 100% Synced with Live index.html (No Passwords + Smart Navigation)
 * =====================================================================
 */
...
// =====================================================================
// 4. NAVIGATION, MODALS & VIEW SWITCHING (No Passwords - Direct Access)
// =====================================================================
...
function switchView(viewId) {
    ...
    if (target) {
        ...
        target.scrollTop = 0;
        if (viewHistory[viewHistory.length - 1] !== target.id) {
            viewHistory.push(target.id);
        }
    }

    const backBtn = document.getElementById("back-btn");
    if (backBtn) {
        const isHome = (viewId === "home" || viewId === "home-view");
...
// Direct Navigation Handler (Password Prompt Removed As Requested)
function askPassword(scope) {
    currentScope = scope;
    if (scope === "CIRCLE" || scope === "DIVISION" || scope === "DC") {
        openProgressReportView(scope);
    } else if (scope === "STOCK") {
...
function setMode(mode) {
    ...
    if (optDaily) optDaily.classList.toggle("active", mode === "DAILY");
    if (optMonthly) optMonthly.classList.toggle("active", mode === "MONTHLY");
    refreshSummary();
}
    ...
    let rowsHtml = rows.map(r => {
        ...
        return `
            <div style="display:grid; grid-template-columns: 2fr 1fr 1fr; padding:8px; border-bottom:1px solid #e2e8f0; font-size:12px;">
                <div style="text-align:left; font-weight:800;">${r.dc_name}</div>
                <div style="text-align:center;">${r.paid_count || 0}</div>
                <div style="color:#166534; font-weight:800; text-align:right;">₹${(r.paid_amount || 0).toLocaleString('en-IN')}</div>
            </div>
        `;
    }).join("");
    ...
    content.innerHTML = `
        <div style="background:#fff; border-radius:12px; border:1px solid #cbd5e1; overflow:hidden; margin-top:12px;">
            ...
            <div style="display:grid; grid-template-columns: 2fr 1fr 1fr; padding:10px; background:#f1f5f9; font-weight:900; font-size:12px; border-top:2px solid #0f766e;">
                <div>TOTAL</div>
                <div style="text-align:center;">${totalPaidCount}</div>
                <div style="color:#166534; text-align:right;">₹${totalPaidAmt.toLocaleString('en-IN')}</div>
            </div>
        </div>
    `;
}
